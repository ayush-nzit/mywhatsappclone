...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ user, joinedRoom, lastRoom }) => {
    ...
    socket.broadcast
      .to(lastRoom.roomID)
      .emit('leaving', `${user.username} left this group`);
    socket.broadcast
      .to(lastRoom.roomId)
      .emit('message', `${user.username} LEFT THIS GROUP`);
    ...
    socket.broadcast
      .to(joinedRoom.roomId)
      .emit('message', `${user.username} joined this group`);
  });
  ...
  socket.on('videoCall', ({ User, joinedRoom }) => {
    ...
    socket.broadcast
      .to(joinedRoom.roomId)
      .emit('videoCall', { User, joinedRoom });
  });

  socket.on('chat', ({ joinedRoom, user, message }) => {
    console.log(joinedRoom, user, message, '///////////////////////');
    // console.log(user)
    io.to(joinedRoom.roomId).emit(
      'chat',
      formatMessage(user.username, message)
    );
  });
